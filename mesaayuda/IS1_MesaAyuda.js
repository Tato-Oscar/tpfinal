...
/*-----------
funcion para hacer el parse de un archivo JSON
*/
...
app.post('/api/loginCliente', (req, res) => {
    ...
    if (!contacto) {
        res.status(400).send({ response: "ERROR", message: "Correo electronico no informado" });
        return;
    }
    ...
    docClient.scan(paramsScan, function (err, data) {
        if (err) {
            ...
            res.status(400).send({ response: "ERROR", message: "DB access error" });
        return;
        }
        ...
        if (data.Items && data.Items.length > 0) {
            const sanitized = data.Items.map(item => ({
                ...item,
            password: 'CENSURADO' // se reemplaza la password real
            }));
        console.log("Scan data:", sanitized);
        } else {
            console.log("Scan data: vacio");
        }      
...
// Funcion para realizar el SCAN de un DB de cliente usando contacto como clave para la b煤squeda (no es clave formal del DB) //
...
app.post('/api/addCliente', (req, res) => {
    ...
    // cambio hecho 3.2: Logica de validacion de unicidad previa al PUT //
    ...
    scanDb(contacto)
        .then(resultDb => {
            ...
            const paramsPut = {
                TableName: "cliente",
                Item: newCliente,
                // <-- cambio hecho 3.4: Elimina ConditionExpression (la validacion ya se hizo con scanDb)
            };
            
            docClient.put(paramsPut, function (err, data) {
                if (err) {
                    // Aqui manejamos otros errores de escritura (no de unicidad)
...
/*---------
Funci贸n para realizar el SCAN de un DB de cliente usando contacto como clave para la b煤squeda (no es clave formal del DB)
*/
...
// se utiliza para borrar los tickets
app.delete('/api/deleteTicket', (req, res) => {
    // Extraer el ID del ticket del cuerpo de la solicitud
    const { id } = req.body; 

    // 1. Validación del ID
    if (!id) {
        // Enviar un error 400 si el ID no se proporciona
        return res.status(400).send({ 
            response: "ERROR", 
            message: "ID de ticket no informado para eliminar" 
        });
    }

    // 2. Parámetros para la operación DeleteItem de DynamoDB
    const params = {
        TableName: "ticket", // Asegúrate de que este es el nombre correcto de tu tabla de tickets
        Key: { 
            // La clave de partición debe coincidir con la de tu tabla
            "id": id 
        }
        // Puedes agregar ConditionExpression si quieres verificar que el ticket exista antes de borrar:
        // ConditionExpression: "attribute_exists(id)"
    };

    // 3. Ejecutar la operación de borrado en DynamoDB
    docClient.delete(params, (err, data) => {
        if (err) {
            console.error("Error al borrar el ticket:", JSON.stringify(err, null, 2));
            // Enviar un error 500 si la base de datos falla
            return res.status(500).send({ 
                response: "ERROR", 
                message: "Error de acceso a DB al intentar eliminar el ticket",
                error: err.message
            });
        } 
        // 4. Respuesta exitosa
        // DynamoDB delete retorna un objeto vacío si es exitoso, sin importar si el ítem existía o no (a menos que uses ConditionExpression)
        
        // --- CORRECCIÓN ---
        // Arreglé un error en la plantilla de string (faltaban las comillas `)
        res.status(200).send({ 
            response: "OK", 
...
app.delete('/api/deleteCliente', (req, res) => {
    // 1. Obtener el ID de la cuenta a borrar desde el body o query params
    const { id } = req.body; // O req.query si lo envías en la URL
    if (!id) {
        return res.status(400).send({ response: "ERROR", message: "Id no informado" });
    }
    // 2. Parámetros para la función de eliminación de tu base de datos (Ej. DynamoDB)
    const params = {
        TableName: "cliente",
        Key: { "id": id }
    };
    // 3. Llamar a la función de la base de datos para eliminar el registro
    docClient.delete(params, (err, data) => {
        if (err) {
            console.error("Error al borrar:", JSON.stringify(err, null, 2));
            return res.status(500).send({ response: "ERROR", message: "Error al borrar en la DB" });
        } else {
            // El registro fue borrado exitosamente
            return res.status(200).send({ response: "OK", message: "Cuenta eliminada exitosamente" });
        }
    });
});
