...
app.post('/api/addCliente', (req, res) => {
    ...
    const newCliente = {
        id: contacto,
        nombre: contacto, // Se usa el email como nombre por defecto
        password: password,
        activo: true,
        registrado: true,
    ...
    docClient.put(paramsPut, function (err, data) {
        if (err) {
            // Si el error es porque el 'id' (email) ya existe, envía el mensaje solicitado
            if (err.code === 'ConditionalCheckFailedException') {
                res.status(400).send({ response: "ERROR", message: "Correo ya existente" });
            } else {
                // Para cualquier otro error de la base de datos
                res.status(400).send({ response: "ERROR", message: "DB error: " + err.message });
            }
        } else {
            // Si todo sale bien
